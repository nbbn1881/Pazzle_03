...
public class GManager : MonoBehaviour
{
    ...
    void CreateObject()
    {
        GameObject obj = Instantiate(prefabObj1,Vector3.zero,Quaternion.identity);
    }
    ...
    void Dealing()
    {
        ...
        float offsetZ = 1.0f;
        int number = 1;
        for (int i =1.5f; i<3; i++)
        {
            for( int j= 0.5f; j<3; j++)
            {
                //Vector3 position = new Vector3(j*offsetX,0.1f,i*offsetZ);
                Instantiate (PieceBase[number], new Vector3(j*offsetX,0.1f,i*offsetZ),Quaternion.identity);
                number++;
                if(number>3)
                {
                    break;
